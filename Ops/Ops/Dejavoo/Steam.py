...
def steam_automate(driver, df1):
    addUser_url = 'https://dvmms.com/steam/admin.aspx?screen_ID=130&ACTION=ADD&Main_Field=6&ret=https%3a%2f%2fdvmms.com%2fsteam%2fadmin.aspx%3fscreen_id%3d101'
    #login and then
    print("Current Dataframe, to be entered: \n", df1)
    driver.get("https://dvmms.com/steam/login.aspx")
    driver.find_element(By.XPATH, '//*[@id="ContentPH_LoginTbl_txtUser"]').send_keys(creds.usernameSpin)
    time.sleep(1)
    driver.find_element(By.XPATH, '//*[@id="ContentPH_LoginTbl_txtPWD"]').send_keys(creds.passwordSpin + Keys.RETURN)
...
def ops_onboard(df, df1, driver):
    ...
    for i, j in enumerate(df['Department']):
        if j == "Operations Team": # and df['Home Base'][i] == 'Orlando':
            print(df1)
            userName.append((df['Email Address Gsuite Primary'][i]).split("@")[0])
...
Here is your access for Dejavoo/DVMSS Steam:
Username: {j}
