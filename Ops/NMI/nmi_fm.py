...
def nmi_automate(driver, df1):
    ...
    login_url = 'https://fattmerchant.transactiongateway.com/resellers/login.php'
    print("Current Dataframe, to be entered: \n", df1)
    driver.get(login_url)
    driver.find_element(By.XPATH, '//*[@id="affiliate-login-username"]').send_keys(creds.usernameNMI)
    time.sleep(1)
    driver.find_element(By.XPATH, '//*[@id="affiliate-login-password"]').send_keys(creds.passwordNMI + Keys.RETURN)
    input("Press Enter after MFA: ") #
    time.sleep(1)
    tid = (driver.current_url).split("tid=")
    addUser_url = f'https://fattmerchant.transactiongateway.com/resellers/options.php?Action=AddAccount&tid={tid[1]}'
